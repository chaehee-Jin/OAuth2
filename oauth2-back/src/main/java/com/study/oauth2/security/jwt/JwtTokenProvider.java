...
public class JwtTokenProvider {
	...
	public String generateAccessToken(Authentication authentication) {
		...
		return Jwts.builder().setSubject("AccessToken").claim("email", authentication).claim("auth", roles)
				.setExpiration(tokenExpiresDate).signWith(key, SignatureAlgorithm.HS256).compact();
	}
...
//		}
		...
		User userEntity = userRepository.findUserByEmail(accessToken);
